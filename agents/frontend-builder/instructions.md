...
**Recommended runtime/provider**: Claude Code. The user's installed skill graph is Claude-Code-shaped (full `vercel:*` plugin, `frontend-design`, `tailwind-v4-shadcn`), and shadcn + Playwright MCPs are first-class in Claude Code's `.mcp.json` flow.
